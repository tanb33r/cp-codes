...
ll n;
ll dy(int i, int op, ll un, ll point) {
    if(un==0) return point;
    if(i==n) return point;
//    debb(i,point);
    ll &ans = dp[i][op];
    if(ans != -1) return ans;

    ll ans2 = dy(i+1,0,un-1, point+a[i]); // add points
    ll ans3 = dy(i+1,1,un-1+a[i], point); // unlocks
...
//    ll ans = 0;
    ...
    ll ans = max(dy(0,0,1,0),dy(0,1,1,0));
    pr(ans);
}
int main() {
